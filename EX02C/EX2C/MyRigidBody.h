...
public:
	/*
	USAGE: Tells if the object is colliding with the incoming one using ARBB
	ARGUMENTS: MyRigidBody* const other -> inspected rigid body
	OUTPUT: are they colliding?
	*/
	bool TestARBB(MyRigidBody* const other);
	/*
	Usage: Constructor
	...
	*/
	...
	/*
	Usage: Returns a point in global coordinates out of a local coordinates one
	Arguments: vector3 a_v3Input -> point to change
	Output: version in global scale of the point
	*/
	vector3 Globalize(vector3 a_v3Input);
	/*
