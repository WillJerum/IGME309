...
void MyCamera::CalculateView(void)
{
	...
	glm::quat combinedRotation = rotationYaw * rotationPitch;

	// Calculate the camera's new forward, target, up, and rightward vectors
	m_v3Forward = glm::normalize(glm::rotate(combinedRotation, glm::vec3(0.0f, 0.0f, -1.0f)));
	m_v3Target = m_v3Position + m_v3Forward;
	m_v3Upward = glm::normalize(glm::rotate(combinedRotation, glm::vec3(0.0f, 1.0f, 0.0f)));
